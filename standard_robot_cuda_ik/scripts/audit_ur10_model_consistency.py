...
URDF = ROOT / "urdf" / "ur10_official.urdf"
SOURCE_JSON = ROOT / "urdf" / "ur10_official_source.json"
JOINT_NAMES = [
    "shoulder_pan_joint",
    "shoulder_lift_joint",
    "elbow_joint",
...
def source_audit() -> list[dict[str, object]]:
    ...
    for path in sorted(set(current)):
        p = Path(path)
        s = str(path)
        role = "discovered"
        used_by = ""
        ...
        rows.append(
            {
                ...
                "risk": risk,
                "notes": "current consolidated project source",
            }
        )
    rows.append(
        {
