...
__device__ __forceinline__ void solve_candidate_v4(const double* T_tgt, const double* q_seed,
  ...
  for (int iter = 0; iter < max_iter; ++iter) {
    ...
    if (kLimitWeight > 0.0 && limit_gradient_mode == 1) {
      double gl[6];
      limit_loss_grad_analytic_v4(q, gl);
      for (int j = 0; j < 6; ++j) g[j] += kLimitWeight * gl[j];
    } else if (kLimitWeight > 0.0) {
      ...
      for (int j = 0; j < 6; ++j) {
        ...
        qp[j] += eps;
        g[j] += kLimitWeight * (limit_loss_v4(qp) - l0) / eps;
      }
    }
    if (precision_mode <= 0) {
      for (int r = 0; r < 6; ++r) {
    ...
    if (kLimitWeight > 0.0) loss_old += kLimitWeight * limit_loss_v4(q);
    ...
    if (kLimitWeight > 0.0) loss_new += kLimitWeight * limit_loss_v4(q_trial);
  ...
  const double total_loss = 0.5 * pose_cost + kLimitWeight * limit_score;
