...
RunSummary run_batch_solver(
  ...
  // Upload data once (all iterations reuse same data)
  ...
  for (int rep = 0; rep < repeat_count; ++rep) {
    ...
    // Convergence data only on first iteration
    if (rep == 0) {
      double sum_pos = 0.0, sum_rot = 0.0, sum_iters = 0.0;
      for (int i = 0; i < N; ++i) {
        double pos_err = h_errors[i * 2 + 0];
        double rot_err = h_errors[i * 2 + 1];
        sum_pos += pos_err; sum_rot += rot_err; sum_iters += h_iters[i];
        if (pos_err < pos_tol && rot_err < rot_tol) summary.converged += 1;
      }
...
void print_summary(const RunSummary& summary, int N) {
  double avg_kernel = 0.0, avg_gpu = 0.0, avg_host = 0.0;
  for (double x : summary.kernel_ms) avg_kernel += x;
  for (double x : summary.gpu_ms) avg_gpu += x;
  for (double x : summary.host_ms) avg_host += x;
  avg_kernel /= static_cast<double>(summary.kernel_ms.size());
  avg_gpu /= static_cast<double>(summary.gpu_ms.size());
  avg_host /= static_cast<double>(summary.host_ms.size());

  std::printf("num_targets=%d\n", N);
  std::printf("repeat_count=%zu\n", summary.kernel_ms.size());
  std::printf("kernel_time_only_ms_mean=%0.6f\n", avg_kernel);
  std::printf("kernel_time_only_ms_p50=%0.6f\n", percentile(summary.kernel_ms, 50.0));
  std::printf("kernel_time_only_ms_p95=%0.6f\n", percentile(summary.kernel_ms, 95.0));
  std::printf("kernel_time_only_ms_p99=%0.6f\n", percentile(summary.kernel_ms, 99.0));
  ...
  std::printf("convergence_rate=%0.6f\n", static_cast<double>(summary.converged) / static_cast<double>(N));
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    if (std::strcmp(argv[i], "--targets") == 0 && i + 1 < argc) {
      ...
      std::printf("Usage: %s --targets <bin> --seeds <bin> [--ablation-level N] [--repeat 30] [--verify-fk]\n", argv[0]);
      return 0;
    }
  }
  ...
  RunSummary summary = run_batch_solver(targets, seeds, repeat_count, max_iter, weight_level, pos_tol, rot_tol);
