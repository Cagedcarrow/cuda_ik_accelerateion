...
// ---- Ablation level compile-time control ----
#ifndef ABLATION_LEVEL
#define ABLATION_LEVEL 6  // Default: full optimized
#endif

// PaddedMat6x8 stride control via ablation level
#if ABLATION_LEVEL < 2
  #define PADDEDMAT_STRIDE 6   // A0/A1: unpadded
#else
  #define PADDEDMAT_STRIDE 8   // A2+: padded
#endif
...
// double arrays with configurable column stride for bank-conflict avoidance.
// All methods are __device__ __forceinline__ for zero overhead.
//
// Default: 8-column padding (stride=8) — row stride = 16 banks (half of 32),
//   adjacent row pairs (r, r+2) use complementary bank sets.
// Ablation (ABLATION_NO_PADDEDMAT): 6-column layout (stride=6) —
//   row stride = 12 banks. Used to measure bank conflict impact via ncu.
//
...
// ============================================================================
struct PaddedMat6x8 {
  double* data;  // Points to __shared__ double[6 * PADDEDMAT_STRIDE]

  __device__ __forceinline__ PaddedMat6x8(double* d) : data(d) {}

  // Access element at (row, col) with configurable stride
...
// ============================================================================
// FloatPaddedMat6x8 — Float version for mixed-precision A7 ablation
// ============================================================================
#if ABLATION_LEVEL == 7
...
// ============================================================================
// Forward kinematics using global memory parameters (for A0 ablation)
//
// Same algorithm as forward_kinematics(), but reads segment origins/axes and
// tool transform from pre-loaded shared memory or passed pointers instead of
// __constant__. This is used for ablation level 0 to measure the cost of
// not using constant memory.
// ============================================================================
#if ABLATION_LEVEL == 0
__device__ __forceinline__ void forward_kinematics_global(
    const double* q, double* T_tip,
    const double* origins, const double* axes, const int* q_index,
    const double* T_wrist3_to_tcp) {
  for (int i = 0; i < 16; ++i) T_tip[i] = (i % 5 == 0) ? 1.0 : 0.0;
  double T_tmp[16], R[16];
  for (int seg = 0; seg < 6; ++seg) {
    ...
    build_rotation_matrix(axes[seg * 3 + 0], axes[seg * 3 + 1], axes[seg * 3 + 2], theta, R);
    mat44_mul(T_tip, R, T_tmp);
    for (int i = 0; i < 16; ++i) T_tip[i] = T_tmp[i];
  }
  mat44_mul(T_tip, T_wrist3_to_tcp, T_tmp);
  for (int i = 0; i < 16; ++i) T_tip[i] = T_tmp[i];
}
#endif
...
__device__ __forceinline__ void pose_error(const double* T_cur, const double* T_tgt,
  ...
  // Rotation error for iterative solving: match the CPU numeric DLS baseline.
  // Use the skew part of R_cur^T * R_tgt without the log-map scaling term.
